...
def load_data(data_path, gParameters):

    print('Loading data...')
    df = (pd.read_csv(data_path,header=None).values).astype('float32')
    df_y = df[:,0]
    df_x = df[:,1:]
    df_x_train, df_x_test, df_y_train, df_y_test = train_test_split(df_x, df_y, test_size=0.2)

    df_train = np.hstack((df_y_train.reshape((df_y_train.shape[0],1)), df_x_train))
    df_test = np.hstack((df_y_test.reshape((df_y_test.shape[0],1)), df_x_test))
    print('done')
